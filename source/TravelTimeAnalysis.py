...
MODE_STYLE = {
    "walk":  {"color": "#66C2A5", "label": "Walk"},    # teal
    "bike":  {"color": "#FC8D62", "label": "Bike"},    # orange
    "drive": {"color": "#8DA0CB", "label": "Drive"},   # lavender-blue
}
...
def _edges_gdf_3857(G: nx.MultiDiGraph) -> gpd.GeoDataFrame:
    """
    Return edges GeoDataFrame in EPSG:3857, compatible with osmnx 1.x/2.x.
    """
    Gp = ox.project_graph(G, to_crs=3857)
    result = ox.graph_to_gdfs(Gp, nodes=False, edges=True, fill_edge_geometry=True)
    if isinstance(result, tuple):
        # defensive if someone flips nodes=True elsewhere
        g_edges = result[1]
    else:
        g_edges = result
    return g_edges
...
def create_outer_boundary(reachable_subgraph: nx.MultiDiGraph, buffer_extra_m: float = 10.0) -> gpd.GeoSeries:
    """
    Convert reachable edges into a single buffered polygon frontier (EPSG:4326).
    Returns empty GeoSeries if nothing reachable.
    """
    if reachable_subgraph is None or reachable_subgraph.number_of_edges() == 0:
        return gpd.GeoSeries([], crs=4326)

    edges3857 = _edges_gdf_3857(reachable_subgraph)
    if edges3857.empty:
        return gpd.GeoSeries([], crs=4326)

    # Reasonable buffer based on edge scale
    lengths = edges3857.length.values
    mean_len = float(np.nanmean(lengths)) if len(lengths) else 20.0
    buf_dist = max(5.0, min(60.0, mean_len * 0.3)) + float(buffer_extra_m)

    buffered = edges3857.buffer(buf_dist)
    merged = unary_union(buffered)

    # Support MultiPolygon -> choose largest by area
    ...
    gseries_3857 = gpd.GeoSeries([poly], crs=3857)
    gseries_4326 = gseries_3857.to_crs(4326)
    return gseries_4326
...
def export_boundary(gseries: gpd.GeoSeries, base_name: str) -> None:
    if gseries is None or len(gseries) == 0:
        ...
        return
    safe = SAFE(base_name)
    gdf = gpd.GeoDataFrame({"name": [base_name]}, geometry=gseries, crs=4326)
...
def _graphs_extent_3857(graphs: List[nx.MultiDiGraph]) -> Tuple[float, float, float, float]:
    ...
    if polys:
        ...
        # Fallback: small patch around Singapore
        fallback = gpd.GeoSeries([Point(103.8198, 1.3521)], crs=4326).to_crs(3857).buffer(15_000).total_bounds
...
def _add_basemap(ax, zoom="auto"):
    # cx.add_basemap(ax, source=cx.providers.CartoDB.PositronNoLabels, crs="EPSG:3414", zoom=zoom)
    cx.add_basemap(ax, source=cx.providers.CartoDB.DarkMatter, crs="EPSG:3414", zoom=zoom)
...
def _draw_scale_bar(ax, extent_3857, loc="lower right", length_m=1000):
    ...
    x0 = x_right - length_m
    ax.plot([x0, x_right], [y, y], lw=3, solid_capstyle="butt", color="#222")
    ax.text(
...
def plot_single_mode_frontiers(
    ...
    for gseries in poi_polys:
        if gseries is None or len(gseries) == 0:
            continue
        gdf3857 = gpd.GeoDataFrame(geometry=gseries, crs=4326).to_crs(3857)
        geom = gdf3857.geometry.iloc[0]
        polys: List[Polygon] = []
        ...
        for poly in polys:
            ...
            # Crisp outline
            _draw_polygon(ax, poly, facecolor="none", edgecolor=color, alpha_fill=0.0, lw=1.6)
